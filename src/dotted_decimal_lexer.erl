...
-module(dotted_decimal_lexer).

-export([string/1,string/2,token/2,token/3,tokens/2,tokens/3]).
-export([format_error/1]).

%% User code. This is placed here to allow extra attributes.
-file("src/dotted_decimal_lexer.xrl", 11).
...
-file("src/dotted_decimal_lexer.erl", 284).
yystate() -> 1.
...
-file("src/dotted_decimal_lexer.xrl", 6).
yyaction_0(TokenChars, TokenLine) ->
     { token, { int, TokenLine, list_to_integer (TokenChars) } } .

-compile({inline,yyaction_1/1}).
-file("src/dotted_decimal_lexer.xrl", 7).
yyaction_1(TokenLine) ->
     { token, { '.', TokenLine } } .
